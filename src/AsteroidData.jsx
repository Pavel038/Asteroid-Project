...
export default function AsteroidData() {
    ...
    useEffect(() => {
        AsteroidService.getAsteroidList().then(response => dispatch({
            type: 'LOAD_ASTEROIDS',
            payload: response
        }));
    }, [])
        ...
        <>
            <DistanceContext value={distanceContextValue}>
                <TasksContext value={{
                    ...
                    destructionList: state.destructionList, dispatch
                }}>
                    <Routes>
                        <Route path="/" element={<App/>}>
                            <Route index element={<AsteroidsPage dataAsteroid={state.asteroidsData}/>}/>

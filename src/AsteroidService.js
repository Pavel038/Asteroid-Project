    ...
    mapDataAsteroid(asteroidList) {
        const mappedAsteroids = Object.entries(asteroidList)?.map(([data, asteroid]) => {
            return asteroid?.map(elem => {
                const lunar = Math.ceil(elem.close_approach_data[0].miss_distance.lunar)
                const {estimated_diameter_max, estimated_diameter_min} = elem.estimated_diameter.meters
                const asteroidSize = Math.ceil((estimated_diameter_max + estimated_diameter_min) / 2)
                const approachData = Math.ceil(elem.close_approach_data[0].miss_distance.kilometers).toLocaleString('ru-RU')
